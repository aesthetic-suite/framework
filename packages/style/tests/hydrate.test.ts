  ...
  it('adds standard and condition rules to the class name cache', () => {
    ...
    expect(renderer.cache.cache).toEqual(
      expect.objectContaining({
        ...
        '@supports (color: blue)@media (width: 350px)@media (width: 500px) { color:blue; }': [
          { className: 't', rank: 0 },
        ],
        '@media (width: 500px):hover { color:red; }': [{ className: 's', rank: 1 }],
        '@media (width: 500px) { margin:10px; }': [{ className: 'r', rank: 2 }],
        '@supports (color: green) { color:green; }': [{ className: 'u', rank: 3 }],
      }),
    );
    expect(renderer.ruleIndex).toBe(21);
  });
