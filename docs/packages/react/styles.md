...
Components are styled with the `useStyles()` hook (preferred) or the `withStyles()` HOC. Both APIs
require a [style sheet](../../development/component-styles.md) that is conditionally rendered to CSS
...
function requires a list of selector names (from the style sheet) in which to render CSS and apply a
class name with, for example.
...
component would look like, using the example from above.
